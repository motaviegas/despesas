...
$stmt = $pdo->prepare("
    ...
    FROM 
        projetos p
    LEFT JOIN 
        categorias c ON p.id = c.projeto_id
    LEFT JOIN 
        despesas d ON p.id = d.projeto_id AND c.id = d.categoria_id
    WHERE 
...
// Calcular métricas adicionais para cada projeto
foreach ($projetos as &$projeto) {
    $projeto['orcamento_remanescente'] = $projeto['orcamento_total'] - $projeto['total_gasto'];
    
    if ($projeto['orcamento_total'] > 0) {
        $projeto['percentagem_execucao'] = ($projeto['total_gasto'] / $projeto['orcamento_total']) * 100;
    } else {
        $projeto['percentagem_execucao'] = 0;
    }
    
    // Determinar a classe da barra de progresso
    if ($projeto['percentagem_execucao'] <= 50) {
        ...
        if ($projeto['percentagem_execucao'] > 100) {
        ...
        $projeto['barra_largura'] = min($projeto['percentagem_execucao'], 100);
    }
}
                            ...
                            <div class="projeto-card <?php echo $projeto['arquivado'] ? 'projeto-arquivado' : ''; ?>">
                                ...
                                <div class="projeto-financeiro">
                                    ...
                                    <div class="financeiro-item">
                                        ...
                                        <span class="financeiro-valor"><?php echo number_format($projeto['total_gasto'], 2, ',', '.'); ?> €</span>
                                    </div>
